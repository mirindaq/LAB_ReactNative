...
const API = 'https://67c83bfd0acf98d0708585f0.mockapi.io/api/food/todo';
...
export default function TaskListScreen() {
  ...
  const searchByKeyword = useCallback(async (keyword: string) => {
    setLoading(true);
    try {
      const url = keyword ? `${API}?des=${keyword}` : API;
      const res = await fetch(url);
      const data = await res.json();
      
      if (Array.isArray(data)) {
        setList(data);
      } else if (typeof data === 'string' && data.toLowerCase() === 'notfound') {
        setList([]);
      } else {
        setList([]);
      }
